...
export default function HistoryItemsManager({ historyItems, setHistoryItems, categories, showToast, setConfirmModal, setConfirmLoading, closeConfirmModal }: Props) {
  ...
  const uploadHistoryFile = async (file: File) => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/history', {
        ...
        body: JSON.stringify({ title: historyTitle.trim(), fileUrl: blob.url, fileType, categoryIds: selectedCategoryIds })
      })
  ...
  const handleOpenHistoryEdit = (item: HistoryItemWithCategories) => {
  ...
  const handleSubmitHistoryEdit = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      const body: Record<string, unknown> = { title: historyEditTitle.trim(), categoryIds: selectedCategoryIds }
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={() => { setShowHistoryForm(true); setHistoryTitle(''); setSelectedCategoryIds([]); setError('') }}
